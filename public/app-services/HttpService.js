app.factory('HttpService', ['$http', '$modal', '$rootScope', '$location', function($http, $modal, $rootScope, $location){
    var service = {};
    // Remove below
    ...
    */
        ...
        $rootScope.logout = function(){
            ...
            $rootScope.user = null;
            $http.get('/api/user/logout-local');
            $location.path('/');
        }
        $rootScope.open_post = function(){
            if(!$rootScope.user) return $rootScope.open_login();
            $location.path('/post');
        }
        $rootScope.open_pages = function(){
            $location.path('/pages');
        }
        $rootScope.open_myaccount = function(){
            $location.path('/my-account');
        }
        service.status = function(user, cb){
            $http.post('/api/user/status', user).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
        service.login = function(user, cb){
            $http.post('/api/user/login-local', user).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
        service.sign = function(user, cb){
            $http.post('/api/user/signup-local', user).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
        service.request = function(info, cb){
            $http.post('/api/user/request', info).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
        service.change = function(info, cb){
            $http.post('/api/user/change', info).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
        service.resetSetting = function (user, cb) {
             $http.post('/api/user/reset_setting', user).then(function(resp){
                cb(resp.data);
            }, handleError('Error getting sales report'));
        }
    ...
    */
    ...
    service.GetPersonalPosts = function () {
        var serializeData = JSON.stringify({ email: $rootScope.user.email });
        var url = "/api/myposts";
        var config = {
            headers : {
                'Content-Type': 'application/json; charset=utf-8',
    ...
    service.GetMyPagesPosts = function () {
        var serializeData = JSON.stringify({ email: $rootScope.user.email });
        var url = "/api/mypages";
        var config = {
            headers : {
