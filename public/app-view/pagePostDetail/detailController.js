app.controller('PagePostDetailController', ['$rootScope','$scope','$location','HttpService','$http','$route','FlashService','$document','$modal','$window', '$routeParams', 'MetaService', function( $rootScope,$scope,$location,HttpService,$http,$route,FlashService,$document,$modal,$window, $routeParams, MetaService){
    var vm = this;
    ...
    $scope.react = function(reaction, postId) {
        if (!$rootScope.user) {
            return alert("You need to be logged in!");
        }
      // Check in the localStorage if this post is already liked
      if (!window.localStorage.getItem(postId + '1')) {
        // Increment post like by one and save it in the localstorage to avoid additional like
        window.localStorage.setItem(postId + '1', true);
        HttpService.ReactPost(postId, reaction)
        .then(function(response) {
          $scope.currentPost.data.reactions[reaction] = $scope.currentPost.data.reactions[reaction] + 1;
        });
      }
    }
    ...
    $scope.likePost = function(postId) {
      ...
      if (!window.localStorage.getItem(postId)) {
        // Increment post like by one and save it in the localstorage to avoid additional like
        window.localStorage.setItem(postId, true);
        HttpService.LikePost(postId)
        .then(function(response) {
          $scope.currentPost.data.like = $scope.currentPost.data.like + 1;
        });
      }
    }
    ...
    $scope.dislikePost = function(postId) {
      ...
      if (!window.localStorage.getItem(postId)) {
        // Increment post like by one and save it in the localstorage to avoid additional like
        window.localStorage.setItem(postId, true);
        HttpService.DislikePost(postId)
        .then(function(response) {
          $scope.currentPost.data.dislike = $scope.currentPost.data.dislike + 1;
        });
      }
    }
   ...
   $scope.addComment = function(id){
        ...
        if( $scope.commentmessage == undefined || $scope.commentmessage == ''){
            FlashService.Error("Please select the Comment!");
            $window.scrollTo(0, 0);
            return;
        }else{
   ...
   $scope.changeReplyMainImage = function(url,parent_index,index){
    ...
    $scope.initController = function () {
        ...
        HttpService.GetAPagePost($routeParams.pageId, $routeParams.postId)
        .then(function(response){
   ...
   $scope.delete = function() {
        $rootScope.modalInstance = $modal.open({
            templateUrl: 'app-view/pagePostDetail/DeleteModal.html'
        });
   };
   ...
   $scope.deletePost = function(){
       $rootScope.modalInstance.close();
       $location.path('/delete/' + $routeParams.postId);
   }
