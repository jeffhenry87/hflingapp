app.controller('MediaPhotoController', ['$rootScope','$scope','$location','HttpService','$http','$route','FlashService','$document','$modal','$window', '$routeParams', 'MetaService', function( $rootScope,$scope,$location,HttpService,$http,$route,FlashService,$document,$modal,$window, $routeParams, MetaService){
    var vm = this;
    ...
    $scope.likePost = function(photoId) {
      ...
      if (!window.localStorage.getItem(photoId)) {
        ...
        .then(function(response) {
          $scope.photoObject.like = $scope.photoObject.like + 1;
        });
      }
    }
    ...
    $scope.dislikePost = function(photoId) {
      ...
      if (!window.localStorage.getItem(photoId)) {
        ...
        .then(function(response) {
          $scope.photoObject.dislike = $scope.photoObject.dislike + 1;
        });
      }
    }
   ...
   $scope.changeReplyMainImage = function(url,parent_index,index){
    ...
    $scope.initController = function () {
        ...
        HttpService.GetPostByPhoto($routeParams.photoId)
        .then(function(response){
            ...
            if (response.status == '200' && response.data["_id"]) {
                ...
                response.data.files.map(function(item) {
                    if (item._id === $routeParams.photoId) {
                        $scope.photoObject = item;
                    }
                });
