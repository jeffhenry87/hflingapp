...
var MAXIMUM_ALLOWED_POST = 9999;//4;
var POSTS_TIMEOUT = 7;
var ADMIN_POSTS_TIMEOUT = 30//30;
...
//    query_params.created = {$gt : deletionDate};
    ...
    Post.findOne(query_params).lean().exec(function (err, post) {
        // if there is an error retrieving, send the error. nothing after res.send(err) will execute
        if (err) {
            res.send(err);
        }
...
exports.read_my_personal_posts = function(req, res) {
    var query_params = url.parse(req.url,true).query;
    query_params.page = {$exists : false};
...
exports.read_my_personal_pages = function(req, res) {
    ...
    Post.find(query_params).populate('page', '_id title profilePic url').exec(function (err, posts) {
        if (err) {
            return res.send(err);
        }
        return res.json(posts);
        ...
        if (/*req.user.email === post.email && */((req.body.sharedData && post.sharedLink && post.sharedData.url !== req.body.sharedData.url) || (req.body.sharedData && !post.sharedLink && req.body.sharedData.url))) {
            ...
            ogs(options, function (error, result) {
                ...
                post.save(function(err, post) {
                    ...
                    if( (req.body.commentmessage || req.body.commentfiles || req.body.commentembed) && req.user){
